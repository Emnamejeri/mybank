...
def my_card():
    delivery_address = input("To order our card please enter a delivery address \nThe address must be composed of: \nThe house number(digits) \nThe street name(string) \nThe Zip/Postal code \nThe city name \nThe Country: \n\n")

    match = re.match(r"^(\d{1,3})\s+(?:[A-Za-z]+\s?){1,3}\s+(\d{3,6})\s+((?:[A-Za-z]+\s?)+)\s+([A-Za-z]+)$", delivery_address)
    if match:
        print("Thank you! The card should reach your specified address in 10 working days.\nBack to the main menu section.....")
        return welcome_page()
    elif delivery_address == "return":
        print("Back to the main menu section.....")
        return welcome_page()
    else:
        print("The provided address is not valid. Please enter a new one and respect the indicated format.")
        return my_card()
...
def help_section():
    print("If you need assistance or have any questions, please don't hesitate to reach out to our dedicated support team - \nWe are always happy to help.")
    support_request = input("Would you like to contact our support team (enter yes or no)? ").lower()
...
def my_data():
    ...
    with open('customerdata.csv', 'r') as file:
        ...
        for row in reader:
            ...
            print("Reminder: You have agreed to our user terms and conditions upon registration \n ------------------------------------")
    return welcome_page()
